...
class SQLBoostOptimizer:
    ...
    def suggest_optimizations(self, query):
        ...
        return {'predicted_execution_time_ms': predicted_time, 'suggestions': suggestions, 'query_complexity': self._calculate_complexity(features), 'features_analysis': self._analyze_features(features)}

    def _calculate_complexity(self, features):
        weights = {'num_joins': 0.15, 'num_subqueries': 0.12, 'num_conditions': 0.1, 'nested_level': 0.08, 'query_length': 0.07, 'num_tables': 0.1, 'has_window_functions': 0.08, 'join_complexity': 0.12}
        score = 0
