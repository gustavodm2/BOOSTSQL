...
class SQLFeatureExtractor:
    ...
    def _count_subqueries(self, query):
        return len(re.findall('\\(\\s*SELECT', query.upper()))
    ...
    def _count_tables(self, query):
        tables = set()
        from_match = re.search('FROM\\s+([^(]+?)(?:\\s+WHERE|\\s+GROUP|\\s+ORDER|\\s+LIMIT|$)', query.upper())
        if from_match:
            tables.update(re.findall('[a-zA-Z_][a-zA-Z0-9_]*', from_match.group(1)))
        return len(tables)
